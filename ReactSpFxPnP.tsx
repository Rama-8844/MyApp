...
export default class ReactSpFxPnP extends React.Component<IReactSpFxPnPProps, IReactSpFxPnP> {
            ...
            <div className={`ms-Grid-row ms-bgColor-neutralLight ms-fontColor-white ${styles.row}`}>
              ...
              <div className="ms-Grid-col ms-u-sm8 block">
                <PeoplePicker
                  ...
                  errorMessage={(this.state.userManagerIDs.length === 0 && this.state.onSubmission === true) ? this.state.required : " "} errorMessageClassName={styles.hideElementManager}
                />
              </div>
  ...
  private _getManager(items: any[]) {
    let userManagerIDs=[]
    for (let item in items) {
          userManagerIDs.push(items[item].id);
        }
    this.setState({ userIDs: userManagerIDs });
    }
  ...
  private createItem(): void {
    ...
    pnp.sp.web.lists.getByTitle("Employee Registration").items.add({
      ...
      Reporting_x0020_ManagerId: {
        results: this.state.userIDs
      }
